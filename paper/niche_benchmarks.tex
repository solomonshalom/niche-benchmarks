...
\medskip
\noindent\textbf{Keywords:} large language models, benchmarks, evaluation, domain specialization, model selection, Goodhart's law, hallucination, position paper
...
This is a position paper. Where we present empirical claims (saturation gaps, contamination evidence, PIF-Bench failure-mode counts), we cite primary sources. Where we make normative claims (about how the evaluation ecosystem should be structured), we mark them as such and engage the strongest opposing views we have found in the literature. The first author is affiliated with R\={o}my, the developer of one of the systems evaluated in PIF-Bench; this conflict is discussed in \Cref{sec:limitations}.
...
For the academic NLP and AI research communities, the implications are methodological. First, position papers and meta-evaluations should engage Tier 2 and Tier 3 benchmarks as legitimate primary sources, not as informal anecdote. SkateBench produces a 20-point spread between two frontier models; that is a real signal, and it warrants real analysis. Second, the venue infrastructure for publishing benchmark work should accommodate the Tier 2 and Tier 3 production styles; a benchmark-card-quality publication with rigorous evaluation but informal exposition should be citable in the same way as a multi-author benchmark paper at NeurIPS. Third, the meta-evaluation literature should focus less on ranking benchmarks against each other and more on developing the discovery, documentation, and aggregation infrastructure that lets a portfolio approach become tractable.

% ── 9. Limitations ───────────────────────────────────────────────────────────
\section{Limitations and Threats to Validity}
\label{sec:limitations}

We identify five limitations of the position taken in this paper.

\paragraph{Position paper, not empirical study.} The central claims of this paper are normative (about how the evaluation ecosystem should be structured) and structural (about the relationships between benchmark types). They are not the kind of claims that can be falsified by a single experiment. Where we draw on empirical evidence (PIF-Bench results, MMLU saturation data, SkateBench rankings), it is illustrative rather than load-bearing. A reader who finds the structural argument unconvincing will not find any single empirical demonstration in this paper that resolves the disagreement.
